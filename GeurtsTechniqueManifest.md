...
- `GeurtsTechniques/GeurtsGitIgnoreTechnique.md` when Game Forge Intelligence sets up or repairs a project-root `.gitignore`

The integration contract governs Game Forge Intelligence and defines the reusable boundary for Geurts-compatible Unity AI integration packages. The Git Ignore Technique owns the exact custom `.gitignore` payload; the integration contract owns when and how the plugin may apply it.
...
Every path between the markers is part of the v0.8.0 repository package and must exist. The Scope column determines whether a file is synchronized into Unity projects, used only in this canonical repository, used as a source template/tool, or used for validation.

<!-- GEURTS-PACKAGE-FILES:BEGIN -->

| Path | Version | Scope |
|---|---:|---|
| `README.md` | 0.8.0 | Repository overview and package changelog; repository-only. |
| `AI_READ_FIRST.md` | 0.8.0 | Canonical AI entry point; synchronized. |
| `AGENTS.md` | 0.7.0 | Canonical-repository Codex entry; repository-only. |
| `GeurtsTechniqueManifest.md` | 0.8.0 | Package/version index; synchronized. |
...
| `GeurtsTechniques/GeurtsFolderStructureDefinition.json` | 0.8.0 | Machine-readable folder-creation automation authority; synchronized. |
| `GeurtsTechniques/GeurtsAIAgentSetupTechnique.md` | 0.8.0 | Normative AI setup and managed-entry technique; synchronized. |
...
| `GeurtsTechniques/GeurtsGameForgeIntelligenceIntegrationContract.md` | 0.9.0 | Normative Game Forge Intelligence integration contract; synchronized. |
| `GeurtsTechniques/GeurtsGitIgnoreTechnique.md` | 1.0.0 | Normative custom project-root `.gitignore` payload; synchronized. |
...
| `Migrations/v0.8.0.md` | 0.8.0 | Migration guide for public synchronization and custom `.gitignore` provisioning; repository-only. |
| `Migrations/v0.7.0.md` | 0.7.0 | Migration guide for v0.4-v0.6 integrations; repository-only. |
| `Tools/GeurtsRepository.json` | 0.8.0 | Repository, access, sparse-layout, and package configuration; tool source. |
| `Tools/BootstrapGeurtsInstructions.ps1` | 0.8.0 | Check/update/validate and atomic synchronization tool; tool source. |
| `Tools/CreateAIAgentInstructionFiles.bat` | 0.7.0 | Compatibility launcher for managed AI setup; tool source. |
| `Tools/ManageGeurtsAgentInstructions.ps1` | 0.8.0 | Native-entry migration and GDD scaffolding manager; tool source. |
| `Tools/CreateGeurtsFolderStructure.bat` | 0.7.0 | Compatibility launcher for definition-driven folder creation; tool source. |
| `Tools/CreateGeurtsFolderStructure.ps1` | 0.8.0 | Definition-driven, create-only folder tool; tool source. |
...
| `Tools/ValidateGeurtsDocumentation.ps1` | 0.8.0 | Package, path, version, authority, and custom-payload validator; validation tool. |
...
| `Tools/Tests/RunAutomationTests.ps1` | 0.8.0 | Temporary-project automation and migration regression suite; validation test. |
...
The public GitHub repository is the canonical documentation authority. A validated copy synchronized into `GeurtsGameForgeDocumentation/` is the normal local read-only source for AI prompts, not an independent authority.

No bundled documentation fallback snapshot is approved for v0.8.0. A future fallback must be explicitly authorized, versioned by package and commit, visibly identified as potentially stale, and subordinate to a newer successfully synchronized canonical copy. The plugin-owned default `.gitignore` is a separate setup asset and is not a documentation fallback.
...
1. Use anonymous read-only access to the public canonical repository and branch `main` by default.
...
4. Validate and reconcile supported contract changes, or report `PLUGIN_UPDATE_REQUIRED` before incompatible documentation becomes active.
5. Stage and validate candidate documentation before atomically replacing the previous valid copy and integration state.
6. Report exact commits and distinct categorized failures without exposing credentials.
7. Consume the folder JSON for creation, respect profile ownership, and never automatically delete managed folders.
8. Report missing GDD scaffolding and manifest drift without automatically writing GDD files; offer a user-approved external authoring or maintenance handoff.
9. Safely classify and migrate native AI entries while preserving user-authored content.
10. Keep synchronized documentation and project-specific GDD authority domains separate.
11. Record the exact documentation commit, package version, contract version, and contract fingerprint used by each AI session.
12. Provision a missing project-root `.gitignore` from the validated custom documentation payload, fall back visibly to a versioned plugin default when necessary, and preserve differing existing files.
