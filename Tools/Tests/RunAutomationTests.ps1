...
try {
    ...
    Assert-True ($draftManifestText -match '(?im)^\*\*Version:\*\*\s*0\.22\.0\s*$' -and $draftManifestText -match '(?im)^\*\*Status:\*\*\s*Draft normative package manifest\s*$' -and $draftReadmeText -match '(?im)^\*\*Status:\*\*\s*Draft technique package\s*$' -and $targetMigrationText -match '(?i)planned transition' -and $targetMigrationText -match '(?i)target-release snapshot' -and $targetMigrationText -notmatch '(?i)(?:v0\.22\.0|package v0\.22\.0)[^\r\n]{0,80}(?:is|was|has been) released') "v0.22.0 remains a Draft target release with a planned snapshot rather than a completed-release claim"
    ...
    Assert-True ([string]$companionContract.schemaVersion -ceq "2.0.0" -and [string]$companionContract.packageVersion -ceq "0.22.0" -and [string]$companionContract.source.repository -ceq "https://github.com/Geurtsy/GeurtsGameForge_Documentation.git" -and [string]$companionContract.source.branch -ceq "main" -and [string]$companionContract.source.selection -ceq "exact-resolved-head-commit-archive") "Companion contract identifies schema 2.0.0, package v0.22.0, and the official exact-main-commit archive source"
    ...
    Assert-True ((@($companionContract.validationEntries) -join "|") -ceq ($expectedCompanionValidationEntries -join "|") -and @($companionContract.validationEntries | Select-Object -Unique).Count -eq 9) "Current companion contract names the exact nine unique package-v0.22.0 source-validation entries"
...
'@
    ...
    foreach ($themeCase in @('missing-technique', 'missing-applicability', 'palette-drift', 'private-theme-fork', 'companion-dependency', 'unexplained-disabled-action')) {
        ...
        $themeFixtureText = [System.IO.File]::ReadAllText($themeFixturePath)
        $expectedThemeFailure = 'Editor UI theme contract'
        if ($themeCase -eq 'missing-technique') {
            Remove-Item -LiteralPath $themeFixturePath
            $expectedThemeFailure = 'Manifest-listed files'
        }
