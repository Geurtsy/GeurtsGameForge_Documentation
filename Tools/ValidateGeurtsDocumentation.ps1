...
try {
    ...
    Add-Check "Package version" ($packageVersion -eq "0.20.2") "Manifest package version is $packageVersion."
    ...
    $draftStatusValid = $manifestText -match '(?im)^\*\*Status:\*\*\s*Draft normative package manifest\s*$' -and $readmeStatusText -match '(?im)^\*\*Status:\*\*\s*Draft technique package\s*$' -and $gfiStatusText -match '(?im)^\*\*Status:\*\*\s*Draft normative technique\s*$' -and $companionStatusText -match '(?im)^\*\*Status:\*\*\s*Draft normative technique\s*$' -and $targetMigrationText -match '(?i)planned transition' -and $targetMigrationText -match '(?i)target-release snapshot' -and $targetMigrationText -notmatch '(?i)(?:v0\.20\.2|package v0\.20\.2)[^\r\n]{0,80}(?:is|was|has been) released'
    Add-Check "Draft target-release status" $draftStatusValid "Package v0.20.2 remains a Draft target release; its migration guide is a planned target-release snapshot, not a completed-release claim."
    ...
    Add-Check "Frozen GFI v2 startup and manual trigger" $manualTriggerValid "The frozen released-consumer contract retains its internally coherent metadata-only launch check and explicit manual Update semantics without authorizing package-v0.20.2 behavior."
    ...
    if ($companionContract) {
        ...
        if ([string]$companionContract.schemaVersion -cne "2.0.0" -or [string]$companionContract.packageVersion -cne "0.20.2" -or [string]$companionContract.packageVersion -cne [string]$packageVersion) { $companionContractFailures.Add("schemaVersion, current packageVersion, or manifest parity is invalid") | Out-Null }
        ...
        if (-not (Test-ExactStringSequence -Actual @($companionContract.validationEntries) -Expected $expectedValidationEntries)) { $companionContractFailures.Add("current validationEntries is not the exact nine-entry package-v0.20.2 list") | Out-Null }
    ...
    Add-Check "Documentation companion closed contract" ($companionContractFailures.Count -eq 0) $(if ($companionContractFailures.Count) { $companionContractFailures -join "; " } else { "The current schema-2.0.0/package-v0.20.2 source contract fixes the official exact-commit archive, one managed directory, nine validation entries, one four-target confirmation, and three exact full-file route mappings." })
    ...
    $companionContractRegistryRow = [regex]::Match($manifestText, '(?m)^\| `GeurtsTechniques/GeurtsDocumentationCompanionContract\.json` \| 0\.20\.2 \| (?<Role>[^|]+) \|\r?$')
