...
try {
    ...
    Add-Check "Package version" ($packageVersion -eq "0.10.0") "Manifest package version is $packageVersion."

    $readmeStatusText = [System.IO.File]::ReadAllText((Join-Path $RepositoryRoot "README.md"))
    $migrationStatusText = [System.IO.File]::ReadAllText((Join-Path $RepositoryRoot "Migrations/v0.10.0.md"))
    ...
    $draftStatusValid = $manifestText -match '(?im)^\*\*Status:\*\*\s*Draft normative package manifest\s*$' -and $readmeStatusText -match '(?im)^\*\*Status:\*\*\s*Draft technique package\s*$' -and $gfiStatusText -match '(?im)^\*\*Status:\*\*\s*Draft normative technique\s*$' -and $migrationStatusText -match '(?i)planned transition' -and $migrationStatusText -match '(?i)target-release snapshot' -and $migrationStatusText -notmatch '(?i)(?:v0\.10\.0|package v0\.10\.0)[^\r\n]{0,80}(?:is|was|has been) released'
    Add-Check "Draft target-release status" $draftStatusValid "Package v0.10.0 remains a Draft target release; its migration guide is a planned target-release snapshot, not a completed-release claim."
    ...
    Add-Check "GFI compatibility redirect" $redirectValid "The legacy path is a short v2 non-normative fail-closed pointer that marks the v1 automatic transaction/recovery lifecycle obsolete and cannot become a second owner."

    $gfiRegistryRow = [regex]::Match($manifestText, '(?m)^\| `GeurtsTechniques/GeurtsGameForgeIntelligenceTechnique\.md` \| 2\.0\.0 \| (?<Role>[^|]+) \|\r?$')
    $gfiLifecycleOwnerRow = [regex]::Match($manifestText, '(?m)^\| Game Forge Intelligence documentation-availability notification, manual update, source-ownership, and project-local consumption boundary \| `GeurtsTechniques/GeurtsGameForgeIntelligenceTechnique\.md` \| (?<When>[^|]+) \|\r?$')
    $activeGfiContractSurfaceText = @($gfiTechniqueText, $manifestText, $aiReadFirstText, $readmeTextForPathTerms, $migrationStatusText) -join [Environment]::NewLine
    ...
    if (-not (Test-Path -LiteralPath $definitionPath -PathType Leaf)) {
        ...
        if ($definition) {
            ...
            foreach ($folder in $folders) {
                ...
                if (-not $profileScopeValid) { $folderFailures.Add("$folderPath differs from its exact v0.9.0 profile scope") | Out-Null }
                $folderDelegations = @{}
    ...
    $folderVersionParityValid = $folderTechniqueVersion -ceq "0.9.0" -and $folderDefinitionVersion -ceq $folderTechniqueVersion -and $folderToolText.Contains('[string]$definition.definitionVersion -ne "0.9.0"') -and $folderToolText.Contains('Folder definition version must be 0.9.0') -and $folderManagerText.Contains('[string]$definition.definitionVersion -ne "0.9.0"') -and $folderManagerText.Contains('Managed setup requires folder definition v0.9.0')
    if (-not $folderVersionParityValid) { $folderFailures.Add("folder technique, JSON definition, and both consuming tools do not agree on definition v0.9.0") | Out-Null }
    ...
    Add-Check "Destructive documentation update warning" $destructiveWarningValid "The exact Update action has a cancel-default warning covering deletion/replacement, lost local edits, untouched project content, no rollback, and missing/incomplete failure risk before any work begins."
    ...
    Add-Check "Validated complete-tree replacement" $completeReplacementValid "The confirmed action validates an ephemeral current-main complete tree before deletion, then creates a detached writable no-Git snapshot; best-effort residue cleanup never gates another Update."
    ...
    Add-Check "Discarded documentation lifecycle exclusion" $discardedLifecycleValid $(if ($missingDiscardedTerms.Count) { "Missing explicit lifecycle exclusions: " + ($missingDiscardedTerms -join ", ") } elseif ($presentObsoleteAffirmative.Count) { "Obsolete affirmative lifecycle language reintroduced: " + ($presentObsoleteAffirmative -join ", ") } else { "The old automatic transaction/recovery system is explicitly forbidden, and legacy Library evidence remains untouched and non-blocking." })

    $draftStatusValid = @(
        @{ Path = "GeurtsTechniques/GeurtsFolderStructureTechnique.md"; Registry = '(?m)^\| `GeurtsTechniques/GeurtsFolderStructureTechnique\.md` \| 0\.9\.0 \| Normative ' },
        @{ Path = "GeurtsTechniques/GeurtsGameDesignDocumentationTechnique.md"; Registry = '(?m)^\| `GeurtsTechniques/GeurtsGameDesignDocumentationTechnique\.md` \| 0\.9\.0 \| Normative ' }
    ...
    $usageDocuments = @("README.md", "AI_READ_FIRST.md", "AGENTS.md", "GeurtsTechniqueManifest.md", "Migrations/v0.10.0.md")
    ...
    Add-Check "Tool invocation and scope documentation" ($usageFailures.Count -eq 0) $(if ($usageFailures.Count) { $usageFailures -join "; " } else { "Every current/v0.10 tool example passes explicit -ProjectRoot; native-only default, create-missing GDD scaffolding, and independent manifest maintenance match the CLI." })
