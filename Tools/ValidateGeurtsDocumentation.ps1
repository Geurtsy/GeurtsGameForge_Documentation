...
try {
    ...
    Add-Check "Package version" ($packageVersion -eq "0.23.2") "Manifest package version is $packageVersion."
    ...
    $draftStatusValid = $manifestText -match '(?im)^\*\*Status:\*\*\s*Draft normative package manifest\s*$' -and $readmeStatusText -match '(?im)^\*\*Status:\*\*\s*Draft technique package\s*$' -and $gfiStatusText -match '(?im)^\*\*Status:\*\*\s*Draft normative technique\s*$' -and $companionStatusText -match '(?im)^\*\*Status:\*\*\s*Draft normative technique\s*$' -and $targetMigrationText -match '(?i)planned transition' -and $targetMigrationText -match '(?i)target-release snapshot' -and $targetMigrationText -notmatch '(?i)(?:v0\.23\.2|package v0\.23\.2)[^\r\n]{0,80}(?:is|was|has been) released'
    Add-Check "Draft target-release status" $draftStatusValid "Package v0.23.2 remains a Draft target release; its migration guide is a planned target-release snapshot, not a completed-release claim."
    ...
    $audienceFailures = New-Object System.Collections.Generic.List[string]
    foreach ($relative in @($listedRecords.Path | Where-Object { $_.EndsWith('.md') } | Sort-Object -Unique)) {
        try {
            $sourceText = [System.IO.File]::ReadAllText((Join-Path $RepositoryRoot $relative))
            foreach ($mode in @('GameUse', 'ForgeDevelopment')) {
                $filtered = ConvertTo-GeurtsAudienceText -Text $sourceText -Mode $mode
                if (-not $relative.StartsWith('Tools/AIAgentInstructionTemplates/') -and -not $filtered.HasFileTag) {
                    throw 'Documentation requires an explicit file audience tag.'
                }
                if ($relative -in @('AI_READ_FIRST.md', 'GeurtsTechniqueManifest.md') -and
                    ($filtered.FileAudience -cne 'AI-READ' -or $filtered.SkippedLines -gt 0)) {
                    throw 'Entry and manifest must remain completely AI-readable.'
                }
            }
        }
    ...
    Add-Check "Frozen GFI v2 startup and manual trigger" $manualTriggerValid "The frozen released-consumer contract retains its internally coherent metadata-only launch check and explicit manual Update semantics without authorizing package-v0.23.2 behavior."
    ...
    if ($companionContract) {
        ...
        if ([string]$companionContract.schemaVersion -cne "2.0.0" -or [string]$companionContract.packageVersion -cne "0.23.2" -or [string]$companionContract.packageVersion -cne [string]$packageVersion) { $companionContractFailures.Add("schemaVersion, current packageVersion, or manifest parity is invalid") | Out-Null }
        ...
        if (-not (Test-ExactStringSequence -Actual @($companionContract.validationEntries) -Expected $expectedValidationEntries)) { $companionContractFailures.Add("current validationEntries is not the exact nine-entry package-v0.23.2 list") | Out-Null }
    ...
    Add-Check "Documentation companion closed contract" ($companionContractFailures.Count -eq 0) $(if ($companionContractFailures.Count) { $companionContractFailures -join "; " } else { "The current schema-2.0.0/package-v0.23.2 source contract fixes the official exact-commit archive, one managed directory, nine validation entries, one four-target confirmation, and three exact full-file route mappings." })
    ...
    $companionTechniqueRegistryRow = [regex]::Match($manifestText, '(?m)^\| `GeurtsTechniques/GeurtsDocumentationCompanionTechnique\.md` \| 2\.1\.2 \| (?<Role>[^|]+) \|\r?$')
    $companionContractRegistryRow = [regex]::Match($manifestText, '(?m)^\| `GeurtsTechniques/GeurtsDocumentationCompanionContract\.json` \| 0\.23\.2 \| (?<Role>[^|]+) \|\r?$')
    ...
    $installedMenusValid = $brickContractText.Contains('**Installed Brick Menus**') -and
        $brickContractText.Contains('Include installed disabled bricks and installed packages absent from the catalogue') -and
        $brickContractText.Contains('Recheck actual package installation before opening a menu') -and
        $brickContractText.Contains('missing, invalid, duplicated, unavailable or throws') -and
        $brickContractText.Contains('BrickEditorMenuAttribute') -and
        $brickContractText.Contains('minimum God dependency of **0.12.0**') -and
    ...
    if (-not (Test-Path -LiteralPath $definitionPath -PathType Leaf)) {
        ...
        if ($definition) {
            ...
            foreach ($folder in $folders) {
                ...
                if (-not $profileScopeValid) { $folderFailures.Add("$folderPath differs from its exact v0.11.0 profile scope") | Out-Null }
                $folderDelegations = @{}
    ...
    if ($themeRegistryRows.Count -ne 1 -or $themeRegistryRows[0].Version -cne "1.2.0" -or -not $trackedSet.Contains($themeRelative)) {
        $themeRoutingFailures.Add("theme technique 1.2.0 must be registered exactly once and tracked") | Out-Null
    }
    ...
    foreach ($requiredThemeRule in @(
        ...
        'God''s Editor assembly owns the canonical public **`ForgeEditorTheme`**',
    ...
    if ($themeText -notmatch '(?im)^\*\*Version:\*\*\s*1\.2\.0\s*$' -or
    ...
    $draftStatusValid = @(
        ...
        @{ Path = "GeurtsTechniques/GeurtsDocumentationCompanionTechnique.md"; Registry = '(?m)^\| `GeurtsTechniques/GeurtsDocumentationCompanionTechnique\.md` \| 2\.1\.2 \| Normative ' }
