...
try {
    ...
    Add-Check "Package version" ($packageVersion -eq "0.16.0") "Manifest package version is $packageVersion."
    ...
    $draftStatusValid = $manifestText -match '(?im)^\*\*Status:\*\*\s*Draft normative package manifest\s*$' -and $readmeStatusText -match '(?im)^\*\*Status:\*\*\s*Draft technique package\s*$' -and $gfiStatusText -match '(?im)^\*\*Status:\*\*\s*Draft normative technique\s*$' -and $companionStatusText -match '(?im)^\*\*Status:\*\*\s*Draft normative technique\s*$' -and $targetMigrationText -match '(?i)planned transition' -and $targetMigrationText -match '(?i)target-release snapshot' -and $targetMigrationText -notmatch '(?i)(?:v0\.16\.0|package v0\.16\.0)[^\r\n]{0,80}(?:is|was|has been) released'
    Add-Check "Draft target-release status" $draftStatusValid "Package v0.16.0 remains a Draft target release; its migration guide is a planned target-release snapshot, not a completed-release claim."
    ...
    Add-Check "Frozen GFI v2 startup and manual trigger" $manualTriggerValid "The frozen released-consumer contract retains its internally coherent metadata-only launch check and explicit manual Update semantics without authorizing package-v0.16.0 behavior."
    ...
    if ($companionContract) {
        ...
        if ([string]$companionContract.schemaVersion -cne "2.0.0" -or [string]$companionContract.packageVersion -cne "0.16.0" -or [string]$companionContract.packageVersion -cne [string]$packageVersion) { $companionContractFailures.Add("schemaVersion, current packageVersion, or manifest parity is invalid") | Out-Null }
        ...
        if (-not (Test-ExactStringSequence -Actual @($companionContract.validationEntries) -Expected $expectedValidationEntries)) { $companionContractFailures.Add("current validationEntries is not the exact eight-entry package-v0.16.0 list") | Out-Null }
    ...
    $companionContractRegistryRow = [regex]::Match($manifestText, '(?m)^\| `GeurtsTechniques/GeurtsDocumentationCompanionContract\.json` \| 0\.16\.0 \| (?<Role>[^|]+) \|\r?$')
